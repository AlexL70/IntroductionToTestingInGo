...
func Test_app_enableCORS(t *testing.T) {
	...
	for _, e := range tests {
		...
		if !e.expectHeader && rr.Header().Get("Access-Control-Allow-Credentials") != "" {
			t.Errorf("%s: expected to header, but got one", e.name)
		}
	}
}
...
func Test_app_authRequired(t *testing.T) {
	...
	for _, e := range tests {
		...
		if e.expectAuthorized && rr.Code == http.StatusUnauthorized {
			t.Errorf("%s: request with good token has not been authorized", e.name)
		}
		if !e.expectAuthorized && rr.Code != http.StatusUnauthorized {
			t.Errorf("%s: request without good token has been authorized", e.name)
		}
	}
}
